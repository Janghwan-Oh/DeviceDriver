...
{
	PDEBUG("%s:%d (unsigned long) gpio_ioremap = 0x%lx\n", __FUNCTION__, __LINE__, gpio_ioremap);
	PDEBUG("%s:%d (unsigned int) gpio_ioremap = 0x%x\n", __FUNCTION__, __LINE__, (unsigned int)gpio_ioremap);
	PDEBUG("%s:%d (unsigned int *) gpio_ioremap = 0x%x\n", __FUNCTION__, __LINE__, (unsigned int *)gpio_ioremap);
	return (volatile unsigned int *)gpio_ioremap;
}
						 ...
						 struct file *filp)
{
	...
	if (!request_mem_region((unsigned long)gpio_ioremap,
							RPI_GPIO_PAGE_SIZE,
							RPI_GPIO_NAME))	{
		PDEBUG("%s:%d Can't get IO Region 0x%x\n\n", __FUNCTION__, __LINE__, (unsigned int)gpio_ioremap);
	}
